#!/usr/bin/env bash
# AGP Tracker v8 — premium redesign: hero, scroll sections, glass, DOF, physical orbs, anti-overlap
# Run from inside ~/agp-tracker :  bash agp-v8.sh
set -e
...
.sec{min-height:100vh;display:flex;flex-direction:column;justify-content:center;
  padding:96px 26px 60px;max-width:1240px;margin:0 auto;scroll-margin-top:0}
...
h2{font-size:clamp(26px,3.6vw,42px);font-weight:700;letter-spacing:-1.2px;line-height:1.12;margin-bottom:12px}
...
.hero{min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;
  text-align:center;padding:110px 26px 70px;position:relative}
.hero h1{font-size:clamp(42px,8.4vw,104px);font-weight:800;letter-spacing:-3.6px;line-height:.94;
  margin-bottom:20px}
...
function build(){
  ...
  <section class="hero sec" id="top">
    <h1 data-rv>WIN BY <span class="g">ASKING</span><br>NOT BURNING.</h1>
    <p data-rv data-d="1">Independent analytics for Agent Grand Prix. Every question, every guess,
      every dollar — reconstructed from the public chain of play.</p>
  ...
  <section class="sec gsec" id="galaxy-s">
    <div class="sec-hd">
      <div class="eyebrow" data-rv>the field</div>
      <h2 data-rv data-d="1">The Strategy Galaxy</h2>
      <p class="lede" data-rv data-d="2">Every attempt is an orb.
        <b>X</b> questions asked · <b style="color:var(--amber)">Y</b> guesses made ·
        <b style="color:var(--blue)">Z</b> points reached · size is spend.
        Guesses cost 5× a question — orbs high on Y paid to skip the thinking.
        <b>Drag to orbit · shift-drag to pan · ⌘/ctrl + scroll to zoom · click an orb.</b></p>
    </div>
  ...
  <section class="sec" id="read-s">
    <div class="sec-hd">
      ...
      <p class="lede" data-rv data-d="2">Written from the data, not by hand. Updates as races land.</p>
    </div>
    ...
    <div style="margin-top:34px">
      ...
      <h2 style="font-size:clamp(20px,2.4vw,28px);margin-bottom:8px" data-rv data-d="1">Does asking or guessing win?</h2>
  ...
  <section class="sec" id="board-s">
    <div class="sec-hd">
      <div class="eyebrow" data-rv>the grid</div>
      <h2 data-rv data-d="1">Racer standings</h2>
      <p class="lede" data-rv data-d="2">Ranked by wins, then points, then cost per win.
        Click a row to fly to them in the galaxy.</p>
    </div>
  ...
  <section class="sec" id="races-s">
    <div class="sec-hd">
      <div class="eyebrow" data-rv>circuits</div>
      <h2 data-rv data-d="1">Every race</h2>
      <p class="lede" data-rv data-d="2"><b>real</b> = how many on the grid actually asked something.</p>
    </div>
...
grep -q "PMREMGenerator"       public/galaxy.js  && echo "  [ok] reflections"      || echo "  [FAIL] reflections"
...
grep -q "class=\"hero sec\""   public/index.html && echo "  [ok] hero"             || echo "  [FAIL] hero"
grep -q "data-cnt"             public/index.html && echo "  [ok] counters"         || echo "  [FAIL] counters"
grep -q "class=\"rail\""       public/index.html && echo "  [ok] section rail"     || echo "  [FAIL] rail"
grep -q "Inter:wght"           public/index.html && echo "  [ok] typography"       || echo "  [FAIL] fonts"
grep -q "api/stream"           src/server.js     && echo "  [ok] SSE"              || echo "  [FAIL] SSE"
echo ""
echo "done.  npm start   →   then HARD refresh: Ctrl+Shift+R"
